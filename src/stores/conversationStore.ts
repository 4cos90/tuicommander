...
type RegistryChatEvent =
  | { kind: "snapshot"; messages: Array<{ role: string; content: string; timestamp: number }>; isStreaming: boolean; streamingText: string; error: string | null; attachedSessionId: string | null; pinned: boolean }
...
function messages(): ConversationMessage[] { return activeConversation().messages(); }
function isStreaming(): boolean { return activeConversation().isStreaming(); }
function streamingText(): string { return activeConversation().streamingText(); }
function error(): string | null { return activeConversation().error(); }
function chatId(): string { return activeConversation().chatId(); }
function sessionUsage(): ChatUsage | null { return activeConversation().sessionUsage(); }
function agentState(): AgentState { return activeConversation().agentState(); }
function currentIteration(): number { return activeConversation().currentIteration(); }
function toolCalls(): ToolCallEntry[] { return activeConversation().toolCalls(); }
function textChunks(): string { return activeConversation().textChunks(); }
function pendingApproval(): PendingApproval | null { return activeConversation().pendingApproval(); }
function agentError(): string | null { return activeConversation().agentError(); }
...
function addUserMessage(content: string): void { addMessage("user", content); }
function addAssistantMessage(content: string): void { addMessage("assistant", content); }
function addSystemMessage(content: string): void { addMessage("system", content); }
  ...
  try {
    ...
    if (tuicSession) {
      try {
        ...
        appLogger.info("conversation", "no saved conversation for session, starting new", { tuicSession, error: String(e) });
      }
      const newId = await invoke<string>("new_conversation_id");
      s.setChatId(newId);
      return;
...
function applyConversationEvent(s: PerTerminalConversationState, event: ConversationEvent): void {
  ...
  switch (event.type) {
    ...
    case "needs_approval":
      s.setPendingApproval({ sessionId: event.tool_name, command: event.command, reason: event.reason });
      break;
...
function accumulateUsageForState(s: PerTerminalConversationState, usage: { input_tokens: number; output_tokens: number }): void {
  s.setSessionUsage((prev) => ({
...
async function sendMessage(text: string, sessionId: string | null): Promise<void> {
  ...
  try {
    ...
    onEvent.onmessage = (event) => applyConversationEvent(s, event);
...
async function cancelStream(): Promise<void> {
  ...
  try {
    const { invoke } = await import("@tauri-apps/api/core");
    await invoke("cancel_conversation", { sessionId: s.chatId() });
  } catch (e) {
...
function processEvent(raw: unknown): void {
  ...
  switch (event.type) {
    ...
    case "text_chunk":
      ...
      break;
    case "tool_call": {
      const entry: ToolCallEntry = { status: "pending", toolName: event.tool_name, args: event.args, startedAt: Date.now() };
      s.setToolCalls((prev) => [...prev, entry]);
      break;
    }
    case "tool_result":
      s.setToolCalls((prev) => {
        ...
        for (let i = updated.length - 1; i >= 0; i--) {
          if (updated[i].toolName === event.tool_name && updated[i].status === "pending") {
            updated[i] = { ...updated[i], status: "done", result: { success: event.success, output: event.output }, duration: Date.now() - updated[i].startedAt };
            break;
          }
        }
        return updated;
    ...
    case "rate_limited":
      ...
      break;
    case "error":
      batch(() => { s.setAgentState("error"); s.setAgentError(event.message); });
      break;
    case "completed":
      batch(() => { s.setAgentState("completed"); s.setCompletionReason(event.reason); });
      break;
  }
}
...
function applyRegistryEvent(s: PerTerminalConversationState, event: RegistryChatEvent): void {
  switch (event.kind) {
    ...
    case "chunk":
      ...
      break;
    case "error":
      batch(() => { s.setIsStreaming(false); s.setStreamingText(""); s.setError(event.message); });
      break;
    case "cleared":
      batch(() => { s.setMessages([]); s.setIsStreaming(false); s.setStreamingText(""); s.setError(null); });
      break;
  }
}
...
async function subscribeToRegistry(targetChatId: string): Promise<void> {
  ...
  try {
    ...
    channel.onmessage = (event) => {
      if (!ready) { buffered.push(event); } else { applyRegistryEvent(s, event); }
    };

    const result = await invoke<{ subscriptionId: number; snapshot: RegistryChatEvent & { kind: "snapshot" } }>(
      "chat_subscribe", { chatId: targetChatId, onEvent: channel },
    );
...
function setStreaming(v: boolean): void { activeConversation().setIsStreaming(v); }
function appendStreamChunk(text: string): void { activeConversation().setStreamingText((prev) => prev + text); }
