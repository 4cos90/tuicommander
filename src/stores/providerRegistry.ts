...
function createProviderRegistryStore() {
	...
	function removeProvider(id: string): void {
		setState(
			produce((s) => {
				const modelIds = s.registry.models.filter((m) => m.provider_id === id).map((m) => m.id);
	...
	function resolveSlot(slot: SlotName): { provider: ProviderEntry; model: ModelEntry } | null {
		const modelId = state.registry.slots[slot];
		if (!modelId) return null;
		const model = state.registry.models.find((m) => m.id === modelId);
		if (!model) return null;
		const provider = state.registry.providers.find((p) => p.id === model.provider_id);
