...
describe("settingsStore", () => {
  ...
  describe("scrollHistoryEnabled", () => {
    it("defaults to false", () => {
      testInScope(() => {
        expect(store.state.scrollHistoryEnabled).toBe(false);
      });
    });
    ...
    it("hydrates from config", async () => {
      mockInvoke.mockResolvedValueOnce({
        shell: null, font_family: "JetBrains Mono", font_size: 14,
        theme: "dark", mcp_server_enabled: false, ide: "vscode",
        scroll_history_enabled: true,
      });

      await testInScopeAsync(async () => {
        await store.hydrate();
        expect(store.state.scrollHistoryEnabled).toBe(true);
      });
    });

    it("defaults to false when missing from config", async () => {
      mockInvoke.mockResolvedValueOnce({
        shell: null, font_family: "JetBrains Mono", font_size: 14,
        theme: "dark", mcp_server_enabled: false, ide: "vscode",
      });
    ...
    it("round-trips through buildConfig via save", async () => {
      await testInScopeAsync(async () => {
        store.setScrollHistoryEnabled(true);
        expect(store.state.scrollHistoryEnabled).toBe(true);
        vi.advanceTimersByTime(600);
        await vi.runAllTimersAsync();
        expect(mockInvoke).toHaveBeenCalledWith("save_config", expect.objectContaining({
          config: expect.objectContaining({ scroll_history_enabled: true }),
        }));
      });
    });

    it("isScrollHistoryEnabled requires both master and sub-flag", () => {
      testInScope(() => {
        expect(store.isScrollHistoryEnabled()).toBe(false);

        store.setScrollHistoryEnabled(true);
        expect(store.isScrollHistoryEnabled()).toBe(false);

        store.setExperimentalFeaturesEnabled(true);
        expect(store.isScrollHistoryEnabled()).toBe(true);

        store.setScrollHistoryEnabled(false);
        expect(store.isScrollHistoryEnabled()).toBe(false);
      });
    });
  });
