...
describe("mcpPopupStore", () => {
	...
	describe("toggleServer", () => {
		it("calls save_mcp_upstreams with patched enabled field", async () => {
			await testInScopeAsync(async () => {
				...
				const betaSaved = savedConfig.servers.find((s: { name: string }) => s.name === "beta");
		...
		it("ignores toggle for unknown server name", async () => {
			await testInScopeAsync(async () => {
				...
				const saveCalls = mockInvoke.mock.calls.filter((c: unknown[]) => c[0] === "save_mcp_upstreams");
	...
	describe("refreshStatus", () => {
		it("updates status without touching servers", async () => {
			await testInScopeAsync(async () => {
				const updatedStatus = {
					upstreams: [{ name: "alpha", status: "failed", transport: { type: "http" }, tool_count: 0 }],
				};
	...
	describe("listenForStatusChanges", () => {
		it("registers upstream-status-changed listener", async () => {
			await testInScopeAsync(async () => {
				...
				expect(mockListen).toHaveBeenCalledWith("upstream-status-changed", expect.any(Function));
	...
	describe("toggleServerForProject", () => {
		it("calls set_project_mcp_upstreams to add server — collapses to null when all enabled", async () => {
			await testInScopeAsync(async () => {
				...
				const call = mockInvoke.mock.calls.find((c: unknown[]) => c[0] === "set_project_mcp_upstreams");
				...
				const threeServerConfig = {
					servers: [
						...MOCK_CONFIG.servers,
						{
							id: "id-gamma",
							name: "gamma",
							transport: { type: "http", url: "http://localhost:9090/mcp" },
							enabled: true,
							timeout_secs: 30,
						},
					],
		...
		it("does nothing when no active repo", async () => {
			await testInScopeAsync(async () => {
				...
				const calls = mockInvoke.mock.calls.filter((c: unknown[]) => c[0] === "set_project_mcp_upstreams");
		...
		it("story 1367-7fb0: effectiveEnabledForRepo reflects toggle without reload", async () => {
			await testInScopeAsync(async () => {
				...
				expect(mockRepoSettingsUpdate).toHaveBeenCalledWith("/test/repo", { mcpUpstreams: null });
			});
		});
