...
describe("useAutoFetch", () => {
	...
	it("handles multiple repos with different intervals", async () => {
		...
		await vi.advanceTimersByTimeAsync(60 * 1000);
		const initialCalls = mockInvoke.mock.calls.filter((c) => c[0] === "run_git_command");
		...
		const calls5min = mockInvoke.mock.calls.filter((c) => c[0] === "run_git_command");
	...
	it("does not double-fetch if startAutoFetch called multiple times", async () => {
		...
		const calls = mockInvoke.mock.calls.filter((c) => c[0] === "run_git_command");
